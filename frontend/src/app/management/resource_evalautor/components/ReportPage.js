...
function ReportPage({ assessmentResult, onBackToConfig }) {
  // Check if assessmentResult and assessmentResult.report are present
  ...
  const { config, report } = assessmentResult; // Destructure config and report

  // Provide sensible defaults if properties might be missing
  const selectedFrameworks = config.frameworks && config.frameworks.length > 0 ? config.frameworks.join(', ') : 'N/A';
  const selectedResourceTypes = config.resourceTypes && config.resourceTypes.length > 0 ? config.resourceTypes.map(type => type.split('::').pop()).join(', ') : 'N/A';
    ...
    if (format === 'json') {
      ...
      if (report.length === 0) {
        ...
        report.forEach((item, index) => {
          ...
          textContent += `  Status: ${item.complianceStatus ? item.complianceStatus.replace('_', ' ') : 'N/A'}\n`; // Handle missing status

          if (item.failedRules && item.failedRules.length > 0) {
            textContent += `  Failed Rules:\n`;
            item.failedRules.forEach(rule => {
              textContent += `    - [${rule.framework}] ${rule.ruleName}: ${rule.details || 'No details'}\n`;
            });
          }
  ...
  const handleDownload = (format) => {
    ...
    const filename = `compliance_report_${cloudProvider.toLowerCase().replace(/\s/g, '_')}_${new Date().toISOString().slice(0,10)}.${format}`;
  ...
  return (
    <div className="report-container">
      ...
      <div className="report-table-container"> {/* Container for table */}
        ...
        {report.length === 0 ? (
          ...
          <table className="compliance-table"> {/* Table structure */}
            <thead>
              <tr>
                <th>Resource Name</th>
                <th>Type</th>
            ...
            <tbody>
              {report.map((item) => (
                <tr key={item.resourceId}>
                  ...
                  <td>{item.resourceType ? item.resourceType.split('::').pop() : 'N/A'}</td> {/* Display just the resource type name */}
                  <td>{item.region || 'N/A'}</td>
                  <td className={getStatusClass(item.complianceStatus)}>{item.complianceStatus ? item.complianceStatus.replace('_', ' ') : 'N/A'}</td>
                  <td>
