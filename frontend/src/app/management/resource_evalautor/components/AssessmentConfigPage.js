...
// Updated function signature to accept Azure credentials as props
function AssessmentConfigPage({
  ...
  // Removed Azure credential state variables from here, as they are now props
  ...
  const handleCloudProviderChange = (event) => {
    ...
    // Removed clearing of Azure credentials here, as they are props from the parent
  };
  ...
  const handleStartAssessment = async () => {
    ...
    // Removed Azure credential client-side validation from here, as it's handled in the parent ResourceEvaluatorPage
    ...
    // Conditionally add Azure credentials from PROPS to the payload
    if (cloudProvider === 'Azure') {
      assessmentDetailsToSend.azureTenantId = azureTenantId;
      assessmentDetailsToSend.azureClientId = azureClientId;
      assessmentDetailsToSend.azureClientSecret = azureClientSecret;
      assessmentDetailsToSend.azureSubscriptionId = azureSubscriptionId;
    }
    ...
    try {
      ...
      // Call the prop function to pass data back to the parent
      // Note: Backend now returns 'resources' key, not 'report' key
      if (onStartAssessmentProcess) { // Defensive check
        onStartAssessmentProcess({
            config: assessmentDetailsToSend,
            report: backendResponse.resources // Pass backend's 'resources' as 'report'
        });
      }
    } catch (err) {
  ...
  return (
    <div className="assessment-config-container">
      ...
      {/* Removed the section for Azure Credentials input fields from here, as they are handled in the parent */}
