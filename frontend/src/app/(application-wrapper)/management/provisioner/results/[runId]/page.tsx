...
export default function ResultsPage() {
  ...
  const handleDestroy = async () => {
    ...
    try {
      ...
      router.push('/management/provisioner'); // Redirect back to form
    } catch (err: any) {
  ...
  return (
    <div className={styles.successContainer}>
      ...
      <div className={styles.actionsContainer}>
        <button onClick={() => router.push('/management/provisioner')} className={styles.buttonSecondary}>
            <FiArrowLeft /> Back to Form
        </button>
        <button onClick={handleDestroy} disabled={isDestroying} className={styles.buttonDanger}>
          {isDestroying ? <><FiLoader className={styles.spinner}/> Deleting...</> : <><FiTrash2 /> Delete Resources</>}
        </button>
      </div>
      {destroyError && <div className={styles.destroyError}><FiAlertTriangle/> {destroyError}</div>}
    </div>
  );
}
